...
`dbus-autoterm` is a Venus OS D-Bus plugin for integrating an Autoterm AIR 2D heater with a Victron GX device. The current plugin is dummy-complete: the full Venus OS service shell, config handling, and D-Bus publication path are in place, while the runtime backend still uses deterministic dummy values by default.

## Current status

- Venus OS plugin shell is implemented.
...
- Venus OS should provide `python3`, `dbus`, `gi.repository.GLib`, and `vedbus`
- `dbus-autoterm` also checks common Victron `velib_python` locations at startup
- if you are running on a non-standard system image, you can also vendor `velib_python` under `/data/apps/dbus-autoterm/ext/velib_python`
...
- GLib-based Venus-style polling when GLib is available
- mock-compatible fallback loop for local development
